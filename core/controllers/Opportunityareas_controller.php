		...
		else
		{
			...
			foreach ($this->model->get_opportunity_areas() as $value)
			{
				...
				'<tr>
					...
					' . ((Functions::check_user_access(['{opportunity_areas_update}']) == true) ? '<td align="right" class="icon"><a data-action="edit_opportunity_area" data-id="' . $value['id'] . '" class="edit"><i class="fas fa-pencil-alt"></i></a></td>' : '') . '
				</tr>';
			}
