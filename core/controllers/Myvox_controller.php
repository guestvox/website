	...
	public function menu($params)
    {
		...
		if (Session::exists_var('myvox') == true AND !empty(Session::get_value('myvox')['account']))
		{
			if (Session::get_value('myvox')['account']['digital_menu'] == true AND Session::get_value('myvox')['account']['settings']['myvox']['menu']['status'] == true)
			{
				...
				{
					if ((Session::get_value('myvox')['account']['type'] == 'hotel' OR Session::get_value('myvox')['account'] == 'restaurant') AND Session::get_value('myvox')['url'] == 'owner' AND !empty(Session::get_value('myvox')['owner']))
						$break = false;
					else if ((Session::get_value('myvox')['account']['type'] == 'restaurant' OR Session::get_value('myvox')['account'] == 'others') AND Session::get_value('myvox')['url'] == 'delivery')
						$break = false;
				}
			}
		}
		...
		if ($break == false)
		{
			...
			else
			{
				...
				$replace = [
					...
					'{$btn_home}' => (Session::get_value('myvox')['url'] == 'owner') ? '<a href="/' . $params[0] . '/myvox/owner/' . Session::get_value('myvox')['owner']['token'] . '"><i class="fas fa-house-user"></i></a>' : '',
					'{$html}' => $html
				];
