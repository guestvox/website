				...
				if (Format::exist_ajax_request() == true)
	            {
	                    ...
	                    if (empty($labels))
	                    {
	                        ...
	                        if (!empty($query))
	                        {
								...
								if ($account['sms'] > 0)
								{
									...
									$sms_text = $sms_subject . $sms_room . $room['name'] . ' ' . $sms_opportunity_area . $_POST['opportunity_area'] . ' ' . $sms_opportunity_type . $_POST['opportunity_type'] . ' ' . $sms_started_date . Functions::get_formatted_date($_POST['started_date'], 'd M y') . ' ' . $sms_started_hour . Functions::get_formatted_hour($_POST['started_hour'], '+ hrs') . ' ' . $sms_location . $_POST['location'] . ' ' . $sms_urgency . ' ' . $sms_confidentiality . ' ' . $sms_description . $_POST['description'];
	                ...
	                if ($_POST['action'] == 'new_survey_answer')
	                {
					   ...
					   if (!empty($_POST['firstname']) AND !isset($_POST['lastname']) OR empty($_POST['lastname']))
						...
						if (!empty($_POST['email']) OR !empty($_POST['phone_lada']) OR !empty($_POST['phone_number']))
						{
							...
							if (!empty($query))
		                    {
								...
								if (!empty($_POST['phone_lada']) AND !empty($_POST['phone_number']))
								{
									if ($account['sms'] > 0)
									{
										...
										if ($account['language'] == 'es')
											$sms_subject = 'Gracias por contestar nuestra encuesta';
										else if ($account['language'] == 'en')
