		...
		else
		{
			...
			foreach ($this->model->get_guest_treatments() as $value)
			{
				...
				'<tr>
					...
					' . ((Functions::check_user_access(['{guest_treatments_update}']) == true) ? '<td align="right" class="icon"><a data-action="edit_guest_treatment" data-id="' . $value['id'] . '" class="edit"><i class="fas fa-pen"></i></a></td>' : '') . '
				</tr>';
			}
