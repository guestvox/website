	...
	public function myvox($params)
	{
		...
		if (!empty($room))
		{
			...
			if (Format::exist_ajax_request() == true)
			{
				...
				if ($_POST['action'] == 'new_vox')
				{
					...
					if (empty($labels))
					{
						...
						if (!empty($query))
						{
							$_POST['assigned_users'] = $this->model->get_users('opportunity_area', $_POST['opportunity_area'], true, $account['id']);
			...
			else
			{
				...
				$template = $this->view->render($this, 'myvox');

				$opt_opportunity_areas = '';

				foreach ($this->model->get_opportunity_areas('request', true, $account['id']) as $value)
					$opt_opportunity_areas .= '<option value="' . $value['id'] . '">' . $value['name'][$account['settings']['language']] . '</option>';

				$opt_locations = '';

				foreach ($this->model->get_locations('request', true, $account['id']) as $value)
					$opt_locations .= '<option value="' . $value['id'] . '">' . $value['name'][$account['settings']['language']] . '</option>';

				$opt_guest_treatments = '';

				foreach ($this->model->get_guest_treatments(true, $account['id']) as $value)
					$opt_guest_treatments .= '<option value="' . $value['id'] . '">' . $value['name'] . '</option>';
