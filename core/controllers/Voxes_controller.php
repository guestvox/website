...
class Voxes_controller extends Controller
{
				...
				if ($vox['type'] == 'request' OR $vox['type'] == 'workorder')
				{
					if ($vox['menu'] == true)
					{
						...
						foreach ($vox['observations']['items'] as $value)
							$p_observations .= '<span>x' . $value['quantity'] . ' - ' . $value['name'][$this->lang] . ' - ' . Functions::get_formatted_currency($value['total'], $value['currency']) . '</span>';
					}
					else
	...
	// 										<div class="label">
	// 											<label required>
	// 												<p>{$lang.time_period} <a data-action="get_help" data-text="{$lang.time_period_help}"><i class="fas fa-question-circle"></i></a></p>
