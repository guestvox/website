			...
			foreach ($this->model->get_users_levels() as $value)
			{
				...
				'<div>
					...
					<div class="buttons flex_right">
						' . ((Functions::check_user_access(['{users_levels_deactivate}','{users_levels_activate}']) == true) ? '<a class="big" data-action="' . (($value['status'] == true) ? 'deactivate_user_level' : 'activate_user_level') . '" data-id="' . $value['id'] . '">' . (($value['status'] == true) ? '<i class="fas fa-ban"></i><span>{$lang.deactivate}</span>' : '<i class="fas fa-check"></i><span>{$lang.activate}</span>') . '</a>' : '') . '
