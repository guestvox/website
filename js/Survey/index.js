...
$(document).ready(function()
{
    ...
    $(document).on('click','[data-action="edit_survey_question"]', function()
    {
        ...
        $.ajax({
            ...
            success: function(response)
            {
                if (response.status == 'success')
                {
                    $('[data-modal="edit_survey_subquestion"]').removeClass('view');
    ...
    $(document).on('click','[data-action="add_survey_subquestion"]', function()
    {
        id = $(this).data('id');
        ...
        $.ajax({
            ...
            success: function(response)
            {
                ...
                {
                    $('[data-modal="edit_survey_subquestion"]').removeClass('view');
                    $('[data-modal="add_survey_subquestion"]').addClass('view');
                }
                else if (response.status == 'error')
                {
                    $('[data-modal="error"]').find('main > p').html(response.message);
    ...
    $('[data-modal="add_survey_subquestion"]').modal().onSuccess(function()
    {
        $('form[name="add_survey_subquestion"]').submit();
    });

    $('form[name="add_survey_subquestion"]').on('submit', function(e)
    {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response)
            {
                ...
                if (response.status == 'success')
                {
                    $('[data-modal="success"]').find('main > p').html(response.message);
                    $('[data-modal="success"]').addClass('view');
                    $('[data-modal="add_survey_subquestion"]').removeClass('view');
                    $('form[name="add_survey_subquestion"]')[0].reset();
                    $('#tbl_survey_questions').find('tbody').html(response.data);
        ...
        $.ajax({
            ...
            success: function(response)
            {
                if (response.status == 'success')
                {
                    ...
                    //     $('form[name="edit_survey_subquestion"]').find('[name="type"]').attr('checked', true);
                    //
                    // if (response.data.type == 'rate')
                    //     $('form[name="edit_survey_subquestion"]').find('[name="type"]').attr('checked', true);

                    $('form[name="edit_survey_subquestion"]').find('[name="survey_subquestion_es"]').val(response.data.es);
                    $('form[name="edit_survey_subquestion"]').find('[name="survey_subquestion_en"]').val(response.data.en);
    ...
    $('[data-modal="edit_survey_subquestion"]').modal().onCancel(function()
    {
        $('label.error').removeClass('error');
        $('p.error').remove();
        $('form[name="edit_survey_subquestion"]')[0].reset();
    });
    ...
    $('form[name="edit_survey_subquestion"]').on('submit', function(e)
    {
        ...
        $.ajax({
            type: 'POST',
            data: form.serialize() + '&id=' + id + '&action=edit_survey_subquestion',
            processData: false,
            cache: false,
            dataType: 'json',
            success: function(response)
